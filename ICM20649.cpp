...
ICM20649::ICM20649(PinName mosi, PinName miso, PinName sclk, PinName cs, PinName irq) : m_SPI(mosi, miso, sclk), m_CS(cs, 1), m_IRQ(irq)
{
    m_IRQ.disable_irq();
    m_IRQ.fall(Callback<void(void)>(this, &ICM20649::irq_handler));
}
 ...
 * @returns true if measurement was successful
 */
bool ICM20649::get_gyroscope(float *gyr_x, float *gyr_y, float *gyr_z)
{
    float buf[3];
 ...
 * @returns true if measurement was successful
 */
bool ICM20649::get_accelerometer(float *acc_x, float *acc_y, float *acc_z)
{
    float buf[3];
 ...
 *    None
 ******************************************************************************/
void ICM20649::read_register(uint16_t addr, int numBytes, uint8_t *data)
{
    uint8_t regAddr;
    uint8_t bank;
 ...
 *    None
 ******************************************************************************/
void ICM20649::write_register(uint16_t addr, uint8_t data)
{
    uint8_t regAddr;
    uint8_t bank;
 ...
 *    None
 ******************************************************************************/
void ICM20649::select_bank(uint8_t bank)
{
    /* Enable chip select */
    m_CS = 0;

    /* clear R/W bit - write, send the address */
    m_SPI.write(ICM20649_REG_BANK_SEL);
 ...
 *    Returns zero on OK, non-zero otherwise
 ******************************************************************************/
uint32_t ICM20649::set_sample_rate(float sampleRate)
{
 ...
 ******************************************************************************/
float ICM20649::set_gyro_sample_rate(float sampleRate)
{
    uint8_t gyroDiv;
    ...
    write_register(ICM20649_REG_GYRO_SMPLRT_DIV, gyroDiv);
 ...
 *    of the finite and discrete number of divider settings
 ******************************************************************************/
float ICM20649::set_accel_sample_rate(float sampleRate)
{
    uint16_t accelDiv;
    ...
    write_register(ICM20649_REG_ACCEL_SMPLRT_DIV_1, (uint8_t) (accelDiv >> 8) );
    write_register(ICM20649_REG_ACCEL_SMPLRT_DIV_2, (uint8_t) (accelDiv & 0xFF) );
 ...
 *
 * @param[in] accelBw
 *    The desired bandwidth value. Use the ICM20649_ACCEL_BW_yHZ macros, which
 *    are defined in the ICM20649.h file. The value of y can be
 *    6, 12, 24, 50, 111, 246, 470 or 1210.
 *
 * @return
 *    Returns zero on OK, non-zero otherwise
 ******************************************************************************/
uint32_t ICM20649::set_accel_bandwidth(uint8_t accelBw)
{
    uint8_t reg;

    /* Read the GYRO_CONFIG_1 register */
    read_register(ICM20649_REG_ACCEL_CONFIG, 1, &reg);
    reg &= ~(ICM20649_MASK_ACCEL_BW);

    /* Write the new bandwidth value to the gyro config register */
    reg |= (accelBw & ICM20649_MASK_ACCEL_BW);
    write_register(ICM20649_REG_ACCEL_CONFIG, reg);
 ...
 *    Returns zero on OK, non-zero otherwise
 ******************************************************************************/
uint32_t ICM20649::read_accel_data(float *accel)
{
    uint8_t rawData[6];
    float accelRes;
    int16_t temp;
    ...
    read_register(ICM20649_REG_ACCEL_XOUT_H_SH, 6, &rawData[0]);
 ...
 *    Returns zero on OK, non-zero otherwise
 ******************************************************************************/
uint32_t ICM20649::read_gyro_data(float *gyro)
{
    uint8_t rawData[6];
    float gyroRes;
    int16_t temp;
 ...
 *    Returns zero on OK, non-zero otherwise
 ******************************************************************************/
uint32_t ICM20649::get_accel_resolution(float *accelRes)
{
    uint8_t reg;
 ...
 *    Returns zero on OK, non-zero otherwise
 ******************************************************************************/
uint32_t ICM20649::get_gyro_resolution(float *gyroRes)
{
    uint8_t reg;
 ...
 *    Returns zero on OK, non-zero otherwise
 ******************************************************************************/
uint32_t ICM20649::set_accel_fullscale(uint8_t accelFs)
{
    uint8_t reg;
 ...
 *    Returns zero on OK, non-zero otherwise
 ******************************************************************************/
uint32_t ICM20649::set_gyro_fullscale(uint8_t gyroFs)
{
    uint8_t reg;
 ...
 *    Returns zero on OK, non-zero otherwise
 ******************************************************************************/
uint32_t ICM20649::enable_sleepmode(bool enable)
{
    uint8_t reg;

    read_register(ICM20649_REG_PWR_MGMT_1, 1, &reg);
    ...
    write_register(ICM20649_REG_PWR_MGMT_1, reg);
 ...
 *    Returns zero on OK, non-zero otherwise
 ******************************************************************************/
uint32_t ICM20649::enable_cyclemode(bool enable)
{
    uint8_t reg;
    ...
    write_register(ICM20649_REG_LP_CONFIG, reg);
...
uint32_t ICM20649::enter_lowpowermode(bool enAccel, bool enGyro, bool enTemp, bool enMag)
{
    ...
    write_register(ICM20649_REG_PWR_MGMT_1, data);
 ...
 *    Returns zero on OK, non-zero otherwise
 ******************************************************************************/
uint32_t ICM20649::enable_irq(bool dataReadyEnable, bool womEnable)
{
    uint8_t intEnable;
    ...
    write_register(ICM20649_REG_INT_ENABLE, intEnable);
    ...
    /* Write value to register */
    write_register(ICM20649_REG_INT_ENABLE_1, intEnable);
 ...
 *    Returns zero on OK, non-zero otherwise
 ******************************************************************************/
uint32_t ICM20649::read_irqstatus(uint32_t *int_status)
{
    uint8_t reg[4];

    read_register(ICM20649_REG_INT_STATUS, 4, reg);
 ...
 ******************************************************************************/
bool ICM20649::is_data_ready(void)
{
    uint8_t status;
    bool ret;

    ret = false;
    read_register(ICM20649_REG_INT_STATUS_1, 1, &status);
 ...
 *    Returns zero on OK, non-zero otherwise
 ******************************************************************************/
uint32_t ICM20649::enable_wake_on_motion(bool enable, uint8_t womThreshold, float sampleRate)
{
    if ( enable ) {
        /* Make sure that the chip is not in sleep */
        enable_sleepmode(false);
        ...
        set_accel_bandwidth(ICM20649_ACCEL_BW_1210HZ);
        ...
        write_register(ICM20649_REG_ACCEL_INTEL_CTRL, ICM20649_BIT_ACCEL_INTEL_EN | ICM20649_BIT_ACCEL_INTEL_MODE);
 ...
 *    Returns zero on OK, non-zero otherwise
 ******************************************************************************/
uint32_t ICM20649::calibrate(float *accelBiasScaled, float *gyroBiasScaled)
{
    uint8_t data[12];
    ...
    /* Disable the FIFO */
    write_register(ICM20649_REG_USER_CTRL, ICM20649_BIT_FIFO_EN);
    write_register(ICM20649_REG_FIFO_MODE, 0x0F);

    /* Enable accelerometer and gyro to store the data in FIFO */
    write_register(ICM20649_REG_FIFO_EN_2, ICM20649_BIT_ACCEL_FIFO_EN | ICM20649_BITS_GYRO_FIFO_EN);

    /* Reset the FIFO */
    write_register(ICM20649_REG_FIFO_RST, 0x0F);
    write_register(ICM20649_REG_FIFO_RST, 0x00);
    ...
    for ( i = 0; i < packetCount; i++ ) {
        read_register(ICM20649_REG_FIFO_R_W, 12, &data[0]);
    ...
    read_register(ICM20649_REG_XG_OFFS_USRH, 2, &data[0]);
    gyroBiasStored[0] = ( (int16_t) (data[0] << 8) | data[1]);
    read_register(ICM20649_REG_YG_OFFS_USRH, 2, &data[0]);
    ...
    write_register(ICM20649_REG_XG_OFFS_USRH, data[0]);
    write_register(ICM20649_REG_XG_OFFS_USRL, data[1]);
    write_register(ICM20649_REG_YG_OFFS_USRH, data[2]);
    write_register(ICM20649_REG_YG_OFFS_USRL, data[3]);
    write_register(ICM20649_REG_ZG_OFFS_USRH, data[4]);
    write_register(ICM20649_REG_ZG_OFFS_USRL, data[5]);
    ...
    read_register(ICM20649_REG_XA_OFFSET_H, 2, &data[0]);
    accelBiasFactory[0] = ( (int16_t) (data[0] << 8) | data[1]);
    read_register(ICM20649_REG_YA_OFFSET_H, 2, &data[0]);
    accelBiasFactory[1] = ( (int16_t) (data[0] << 8) | data[1]);
    read_register(ICM20649_REG_ZA_OFFSET_H, 2, &data[0]);
    ...
    write_register(ICM20649_REG_XA_OFFSET_H, data[0]);
    write_register(ICM20649_REG_XA_OFFSET_L, data[1]);
    write_register(ICM20649_REG_YA_OFFSET_H, data[2]);
    write_register(ICM20649_REG_YA_OFFSET_L, data[3]);
    write_register(ICM20649_REG_ZA_OFFSET_H, data[4]);
    write_register(ICM20649_REG_ZA_OFFSET_L, data[5]);
 ...
 *    Returns zero on OK, non-zero otherwise
 ******************************************************************************/
uint32_t ICM20649::calibrate_gyro(float *gyroBiasScaled)
{
    uint8_t data[12];
    uint16_t i, packetCount, fifoCount;
    int32_t gyroBias[3] = { 0, 0, 0 };
    ...
    /* Enable accelerometer and gyro to store the data in FIFO */
    write_register(ICM20649_REG_FIFO_EN_2, ICM20649_BITS_GYRO_FIFO_EN);
 ...
 *    Returns zero on OK, non-zero otherwise
 ******************************************************************************/
uint32_t ICM20649::read_temperature(float *temperature)
{
    uint8_t data[2];
    int16_t raw_temp;

    /* Read temperature registers */
    read_register(ICM20649_REG_TEMPERATURE_H, 2, data);
 ...
 *    Returns zero on OK, non-zero otherwise
 ******************************************************************************/
uint32_t ICM20649::get_device_id(uint8_t *device_id)
{
    read_register(ICM20649_REG_WHO_AM_I, 1, device_id);

    return ICM20649_OK;
}

void ICM20649::irq_handler(void)
{
